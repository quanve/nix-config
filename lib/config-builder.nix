...
let
  shouldBeExecutable =
    ...
    builtins.any (
      ...
      if lib.isString pattern then
        lib.hasSuffix pattern filePath || pattern == fileName || pattern == relativePath
      else if lib.isList pattern then
        let
          regex = builtins.head pattern;
        in
  ...
  processDirectory =
    ...
    let
      ...
      processEntry =
        ...
        let
          ...
          relativeFromRoot =
            if cfg.target == cfg.name then "${cfg.name}/${name}" else "${cfg.target}/${name}";
        in
    ...
    in
    builtins.foldl' (acc: entryName: acc // (processEntry entryName contents.${entryName})) { } (
  ...
  buildMany = configsList: builtins.foldl' (acc: cfg: acc // buildConfig cfg) { } configsList;
